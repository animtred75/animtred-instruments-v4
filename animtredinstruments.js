/*
Midi Sound Engine v4.1.2

a Cartoon the Sounds, record OBS Studio a Edit program: Adobe Premiere

My Soundbank 13,861,496 byte

2023/4/7 Anim Tred Studio, LLC
*/
!function(d) {
	var j = {};
	function h(k) {
	...
	'./src/assets/instruments/organ.wav': function(z, x, c) {
		z.exports = c.path + 'https://assets.scratch.mit.edu/internalapi/asset/6404e29622b6443eae9ad68be58cd962.wav/get/'
	},
					...
					if (jj == 255) {
						...
						if (jj > 239) {
							...
							case 14:
								...
								break;
							default: 
								throw new Error("Unknown event" + ((jj > 127) ? "" : " (running)") + ":" + jj);
							}
						}
					}
				}
	...
	'./src/track.js': function(z, x, c) {
		var he = function(a, o, f) {
			...
			if (!("last"in this.decodeControllers)) {
				var controllersLast = new Array(2048);
		...
		he.prototype.update = function() {
			...
			if (this.decodePitchBends.tick.length > 0) {
				while (this.pitchBendIndex < this.decodePitchBends.tick.length && _t >= this.decodePitchBends.tick[this.pitchBendIndex]) {
					this.mse.effects.pitchBendChannel[this.decodePitchBends.channel[this.pitchBendIndex]] = this.decodePitchBends.pitch[this.pitchBendIndex];
				...
				while (this.pitchBendIndex >= 0 && _t < this.decodePitchBends.tick[this.pitchBendIndex]) {
					...
					if (k == null) {
						this.mse.effects.pitchBendChannel[this.decodePitchBends.channel[this.pitchBendIndex]] = 8192;
					} else {
						this.mse.effects.pitchBendChannel[this.decodePitchBends.channel[k]] = this.decodePitchBends.pitch[k];
					}
					this.pitchBendIndex -= 1;
				}
				this.pitchBendIndex += 1;
			...
			if (this.decodeProgramChanges.tick.length > 0) {
				while (this.programIndex < this.decodeProgramChanges.tick.length && _t >= this.decodeProgramChanges.tick[this.programIndex]) {
					this.mse.effects.programChannel[this.decodeProgramChanges.channel[this.programIndex]] = this.decodeProgramChanges.program[this.programIndex];
					this.programIndex += 1;
				}
				this.programIndex -= 1;
				while (this.programIndex >= 0 && _t < this.decodeProgramChanges.tick[this.programIndex]) {
					var k = this.decodeProgramChanges.last[this.programIndex];
					if (k == null) {
						this.mse.effects.programChannel[this.decodeProgramChanges.channel[this.programIndex]] = 0;
					} else {
						this.mse.effects.programChannel[this.decodeProgramChanges.channel[k]] = this.decodeProgramChanges.program[k];
					}
					this.programIndex -= 1;
				}
				this.programIndex += 1;
		...
		he.prototype.setController = function(g, h, j, k) {
			switch (h) {
			case 6:
				this.mse.effects.dataEntryMSB[g] = j;
				break;
			case 7:
				this.mse.effects.volumeChannel[g] = j;
				if (k) {
					this.mse.effects.volumeChannel[g] = 100;
				}
				break;
			case 10:
				this.mse.effects.panChannel[g] = j;
				if (k) {
					this.mse.effects.panChannel[g] = 63;
				}
				break;
			case 11:
				this.mse.effects.expressionChannel[g] = j;
				if (k) {
					this.mse.effects.expressionChannel[g] = 127;
				}
				break;
			case 38:
				this.mse.effects.dataEntryLSB[g] = j;
				break;
			case 64:
				this.mse.effects.sustainPedalChannel[g] = j;
				if (k) {
					this.mse.effects.sustainPedalChannel[g] = 0;
				}
				if (this.mse.isEffect) {
					for (let i = 0; i < this.mse.notesPlaying.length; i++) {
						...
						if (g == c.channel) {
							if (this.mse.effects.sustainPedalChannel[g] > 0) {
				...
				break;
			case 100:
				this.mse.effects.registeredParameterLSBchannel[g] = j;
				break;
			case 101:
				this.mse.effects.registeredParameterMSBchannel[g] = j;
				break;
			case 120:
			case 123:
			case 126:
	...
	'./src/player.js': function(z, x, c) {
		...
		,
		ke.prototype._decodeAudio = function(k) {
			var h = this;
		...
		,
		ke.prototype._vidAudio = function(j, n) {
			if (Array.isArray(j)) {
		...
		ke.prototype.playNote = function(n) {
			if (n.type == 1) {
				...
				const w = v.gain;
				var f = n.volume * this._vidAudio(e.volume, n.pitch);
				w.value = f;
		...
		ke.prototype._playNotes = function(n, i) {
			...
			if (n[0] == 1) {
				...
				if (v.loop = span.loop)
					...
					v.loopEnd = 5;
				v.connect(b);
				var m = (span.releasePatch ? (n[2] + (span.releasePatch - 60)) : n[2]);
				v.playbackRate.value = Math.pow(2, ((m - 60) / 12));
				const fs = b.gain;
				var volume = (n[3] / 99) * this._vidAudio(span.volume, n[2]);
				const _o = (n[6] - n[7]);
				fs.value = 0;
				...
				b.connect(this.node);
				const _o = (n[5] - n[6]);
				var volume = ((n[2] / 99) * (span.volume || 1)) * 0.8;
				var te = {
					_pitch: n[1],
					_volume: n[2],
					_instrument: 0,
		...
		,
		ke.prototype._getSustainPedalChannel = function(channel) {
			return this.effects.sustainPedalChannel[channel]
		}
		,
		ke.prototype._getPanChannel = function(channel) {
			return (this.effects.panChannel[channel] - 64) / 63 * 100
		}
		,
		ke.prototype._getVolumeChannel = function(channel) {
			return (this.effects.volumeChannel[channel] * (this.effects.expressionChannel[channel] / 127)) / 100
		}
		,
		ke.prototype._getPitchBend = function(channel) {
			if (channel == 9) return 0;
			var _ = this.effects.dataEntryMSB[channel];
			return (((this.effects.pitchBendChannel[channel] - 8192) / 8192) * _)
		}
		,
		...
		,
		ke.prototype._stepUpdateTrack = function() {
			if (this._currentTimeLast == this.currentTime)
				return;
			var f = this._currentTimeLast * 1000000
			  , d = this.currentTime * 1000000;
			if (d > f) {
				while (true) {
					if (d > f) {
						this._stepUpdateTimeTrack(f, 1)
					} else {
						f = d,
						this._stepUpdateTimeTrack(f, 1);
						break
		...
		,
		ke.prototype._stepUpdateTimeTrack = function(g, h) {
			if (h !== 0) {
				var j = 0;
				for (var i = 0; i < this.trackQueue.length; i++) {
		...
		,
		ke.prototype._stepPlayingNote = function() {
			var jl = [];
			for (var i = 0; i < this.trackQueue.length; i++) {
				var d = this.trackQueue[i];
				var gg = d.decodeNoteQueue;
				var fh = d.getNote(this.muteMusicr);
				for (var i1 = 0; i1 < fh.length; i1++) {
					var n = fh[i1]
					  , a = gg.channel[n]
					  , s = (a == 9 ? 0 : (this.effects.programChannel[a] + 1))
					  , f = gg.tickOff[n]
					  , g = gg.tickOn[n]
					  , h = gg.pitch[n]
					  , volume = gg.velocity[n] - 1;
					if (!volume)
						continue;
					if (s > 0) {
						if (this.onplaynote)
						...
						jl.push([1, s, h, volume, a, d.trackNumber, f, g]);
					} else {
						if (this.onplaynote) {
							this.onplaynote({
								drum: h,
						...
						jl.push([0, h, volume, a, d.trackNumber, f, g]);
					}
				}
			}
		...
		,
		ke.prototype._stepNotesPlaying = function() {
